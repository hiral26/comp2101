...
#for i in $(ip addr show | awk '$1 == "inet" && $3 == "brd" { sub (/\/.*/,""); print $2 }');
#do
################
#echo "interfaces are:$i" ;
#done
for i in $(ip link show | awk '{print $2}')
do
echo "interface is: $i"
done
int_number=$(ip link show | awk '{print $2}' | wc -l)
echo "interface numbers are $int_number"
#interface=$ ip link show | awk '{print $2}'
#echo $interface
# Data Gathering
################
