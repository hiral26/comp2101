...
test -r /etc/resolv.conf && echo "/etc/resolv.conf file is readable"
...
test -d /tmp && echo"tmp is directory."
# TASK 4: Add a test to see if the /tmp directory is readable
test -r /tmp && echo"tmp is readable."
...
test -x /tmp && echo"tmp is executable."
...
test -v USER && echo "variable exists"
